...
def my_render(gaussians, pipeline, background, intrinsics, dims, R, T):
    ...
    res_pkg = render(view_matrix, cam, gaussians, pipeline, background)
    return res_pkg

def render(viewmatrix, viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None):
    """
    Render the scene. 
