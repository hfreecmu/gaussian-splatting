...
def compute_ratios(conv_2d, points_xy, indices_mask, sam_mask, h, w):
    ...
    for k in range(len(index)):
        ...
        x_point = torch.clip(x_point, 0, w-1)
        y_point = torch.clip(y_point, 0, h-1)
        if torch.is_floating_point(x_point):
            x_point = torch.round(x_point).long()
            y_point = torch.round(y_point).long()
        else:
            x_point = x_point.long()
